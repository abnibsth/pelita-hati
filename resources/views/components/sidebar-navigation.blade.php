...
<div class="space-y-1">
    ...
    {{-- Admin (Kota, Kecamatan, Kelurahan) --}}
    @if($canManageWilayah || $canManageUsers)
    ...
    {{-- Kader --}}
    @if($isKader)
        {{-- Directly matching the hardcoded layout for Kader with no extra headers --}}
        <a href="{{ route($routes['balita']) }}" class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('*.balita.*') ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-700 hover:bg-gray-100' }}">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path></svg>
            <span>Data Balita</span>
        </a>
        
        <a href="{{ route($routes['kehadiran']) }}" class="flex items-center space-x-3 px-3 py-2 rounded-lg {{ request()->routeIs('*.kehadiran.*') ? 'bg-primary-50 text-primary-700 font-medium' : 'text-gray-700 hover:bg-gray-100' }}">
