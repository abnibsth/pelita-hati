...
@section('title', 'Edit Data Imunisasi - SiPosyandu')
@section('page-title', 'Edit Data Imunisasi')
        ...
        <div class="mb-6 p-4 bg-blue-50 rounded-lg">
            <div class="flex items-center space-x-3">
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-600">NIK: {{ $record->balita->nik }} | Umur: {{ $record->balita->age_months }} bulan</p>
                </div>
            </div>
        </div>

        <form action="{{ route($imunisasiPrefix . '.update', $record) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <textarea name="keterangan" id="keterangan" rows="3"
                    ...
                    placeholder="Catatan tambahan (opsional)">{{ old('keterangan', $record->keterangan) }}</textarea>
                @error('keterangan')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
            </div>

            <div class="p-4 bg-green-50 border border-green-200 rounded-lg">
                <p class="text-sm text-green-800">
                    <strong>✓</strong> Perubahan akan disimpan pada riwayat imunisasi balita.
                </p>
            </div>
