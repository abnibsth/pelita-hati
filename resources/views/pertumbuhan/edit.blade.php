...
@section('title', 'Edit Data Pertumbuhan - SiPosyandu')
@section('page-title', 'Edit Data Penimbangan')
...
@section('sidebar')
    <a href="{{ route($balitaPrefix . '.show', $record->balita) }}" class="flex items-center space-x-3 px-3 py-2 rounded-lg text-gray-700 hover:bg-gray-100 font-medium">
    ...
    <div class="mb-6 p-4 bg-blue-50 rounded-lg">
        <div class="flex items-center space-x-3">
            <div class="w-12 h-12 bg-primary-100 rounded-full flex items-center justify-center">
                <span class="text-primary-600 font-bold">{{ substr($record->balita->name, 0, 1) }}</span>
            </div>
            <div>
                <p class="font-semibold">{{ $record->balita->name }}</p>
                <p class="text-sm text-gray-600">Umur: {{ $record->balita->age_months }} bulan | {{ $record->balita->gender === 'L' ? 'Laki-laki' : 'Perempuan' }}</p>
            </div>
        </div>
    </div>

    <form action="{{ route($pertumbuhanPrefix . '.update', $record) }}" method="POST" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="number" name="berat_badan" id="berat_badan" step="0.01" min="0" max="50" required
                    ...
                    value="{{ old('berat_badan', $record->berat_badan) }}" placeholder="Contoh: 12.5">
            ...
            <div>
                ...
                <input type="number" name="tinggi_badan" id="tinggi_badan" step="0.1" min="30" max="150" required
                    ...
                    value="{{ old('tinggi_badan', $record->tinggi_badan) }}" placeholder="Contoh: 85.5">
            ...
            <div>
                ...
                <input type="number" name="lingkar_kepala" id="lingkar_kepala" step="0.1" min="20" max="80"
                    ...
                    value="{{ old('lingkar_kepala', $record->lingkar_kepala) }}" placeholder="Contoh: 48.0">
            ...
            <div>
                ...
                <input type="number" name="lingkar_lengan_atas" id="lingkar_lengan_atas" step="0.1" min="5" max="50"
                    ...
                    value="{{ old('lingkar_lengan_atas', $record->lingkar_lengan_atas) }}" placeholder="Contoh: 14.5">
        ...
        <div>
            ...
            <textarea name="catatan" id="catatan" rows="3"
                ...
                placeholder="Keluhan, riwayat penyakit, atau catatan lainnya">{{ old('catatan', $record->catatan) }}</textarea>
            @error('catatan')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
        </div>

        <!-- Status Gizi (Readonly - Auto Calculated) -->
        <div class="p-4 bg-gray-50 border border-gray-200 rounded-lg">
            <p class="text-sm font-medium text-gray-700 mb-2">Status Gizi Saat Ini:</p>
            <div class="flex items-center space-x-4">
                ...
                <span class="text-sm text-gray-600">Z-Score BB/U: <strong>{{ $record->z_score_bbu }}</strong></span>
            </div>
            <p class="text-xs text-gray-500 mt-2">Status gizi dan Z-Score akan dihitung ulang setelah data disimpan.</p>
        </div>

        <!-- Submit Buttons -->
        <div class="flex items-center justify-end space-x-4 pt-6 border-t border-gray-200">
            <a href="{{ route($balitaPrefix . '.show', $record->balita) }}" class="px-4 py-2 text-gray-700 hover:bg-gray-100 rounded-lg transition">Batal</a>
